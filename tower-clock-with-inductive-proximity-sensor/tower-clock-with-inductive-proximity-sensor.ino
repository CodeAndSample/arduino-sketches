...
bool timeAddedInMarch = false; // Counter to keep track of proximity detections
...
void loop()
{
    ...
    if (read_button() == switched)
    {
        ...
        {
            // TODO: Here will go the code for the activation of the magnet using a relay
        ...
        digitalWrite(LED, led_status);
        // if winter time changes to summer time on the 31st of March of 2024
        // add 1620 to the counter. This is roughly number of oscillations between 2:00 and 3:00
        // if the time has not been added yet
        if (now.month() == 3 && now.day() == 31 && now.year() == 2024 && timeAddedInMarch == false)
        {
            counter = counter + 1620;
            timeAddedInMarch = true;
        }
    }
    else
    {
